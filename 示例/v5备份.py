...
class AnalysisResult:
    ...
    project_name: str
    tier: str
    tier_probabilities: Dict[str, float]  # 改为概率分布
...
class PotentialCalculator:
    ...
    def calculate(self, data: pd.DataFrame, tier: str) -> Dict:
        ...
        return {
            ...
            'remaining_space': min(100, max(0, remaining)),  # 限制在0-100之间
...
"""
    ...
    def _plot_backtest_results(self, ax, result: AnalysisResult):
        ...
        if not metrics:
            ax.text(0.5, 0.5, '无有效回测指标', ha='center', va='center')
            return
        ...
        # 绘制双柱状图
        bars1 = ax.bar(x - width/2, mape_values, width, label='MAPE误差(%)', 
