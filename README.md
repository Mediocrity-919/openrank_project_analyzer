...
**GitHub项目深度分析器**允许您直接使用Web界面进行操作，或直接运行python脚本进行分析。

**Web界面操作**：

1. 双击或在终端执行 [`run.bat`](./run.bat) 启动后端服务器。
2. 打开浏览器，访问 `http://localhost:5501` 进入前端界面（这一步会随着 `run.bat` 的运行自动完成）。
3. 在前端界面选择需要的操作即可。

**python脚本操作**：

1. 确保已安装Python 3.11及以上版本，并安装所需依赖（可通过命令行 `pip install -r ./python/scripts/requirements.txt` 安装）。
2. 在命令行中导航到 [`./python/`](./python/) 目录。
3. 运行以下命令进行单个项目分析：
...
此外，也正是因为调用了 GitHub API，在国内可能会遇到访问速度较慢或请求受限的问题(*一般单次访问的耗时为40s*)，建议使用代理或VPN以获得更好的体验。
...
- [**Mediocrity-919**](https://github.com/Mediocrity-919)
  - 项目总体设计与架构
  - 后端数据分析与处理
  - 健康评估模块设计与实现
  - 前端界面设计与实现
- [**ShrHang**](https://github.com/ShrHang)
  - 文档编写
  - 测试与质量保证
  - PPT制作与项目展示
  - 汇报视频录制与剪辑
