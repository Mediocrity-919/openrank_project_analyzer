...
class PotentialCalculator:
    ...
    def _estimate_ceiling_pessimistic(self, data: pd.DataFrame) -> float:
        ...
        if len(openrank) >= 12:
            ...
            trend_based = min(historical_max * 1.2, future_6m)
            ceiling = min(historical_max * 2.0, trend_based)
        else:
            ceiling = historical_max * 1.5

        return min(100, max(future_6m, ceiling))
...
class ProjectAnalyzerV45:
    ...
    def generate_report(self, result: AnalysisResult) -> str:
        ...
        report = f"""
{'═'*70}
                    {result.project_name} 深度诊断报告 (v4.5)
                    GitHub 项目分析器 - 算法升级版
{'═'*70}
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("  GitHub 项目深度分析器 v4.5 - 算法升级版")
    print("  GMM分层 + Prophet预测 + AHP评估 + 回测验证")
    print("="*60 + "\n")
    
    url = input("请输入 GitHub 项目地址: ").strip()
    if not url:
        url = "facebook/react"  # 默认示例
    
    # 询问 GitHub Token（可选）
    token = input("请输入 GitHub API Token (可选，直接回车跳过): ").strip()
    if not token:
        token = None
